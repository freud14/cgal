...
public:
  ...
  WSPD(int d, double separation_ratio, InputIterator begin, InputIterator end) : s(separation_ratio), split_tree(d, begin, end) { }

  template <class OutputIterator>
  OutputIterator compute(OutputIterator result) const {
    compute(split_tree.root(), result);
    return result;
  }

  template <class InputIterator>
  void set(int d, InputIterator begin, InputIterator end) {
    split_tree = Split_tree(d, begin, end);
  }
